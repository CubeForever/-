...
# 全透明色——此颜色的所有像素在窗口上完全不可见
TRANS_COLOR = '#010101'  # 全透明色：此颜色像素完全不可见 + 鼠标穿透

# ======================== 主题色板（4套）======================
THEMES = {
    '极光': {
        'page': TRANS_COLOR, 'card': '#141A3A', 'border': '#448AFF',
    ...
    '霓虹': {
        'page': TRANS_COLOR, 'card': '#1A0033', 'border': '#E040FB',
    ...
    '樱花': {
        'page': TRANS_COLOR, 'card': '#FFFFFF', 'border': '#F48FB1',
    ...
    '海洋': {
        'page': TRANS_COLOR, 'card': '#122039', 'border': '#4FC3F7',
...
class LifeRecorder:
    ...
    def __init__(self, root):
        ...
        # === 全透明引擎：直接调用 Win32 API ===
        # 绕过 Tk 的透明色实现，避免兼容性问题
        try:
            import ctypes

            hwnd = self.root.winfo_id()  # 窗口句柄

            # 1. 设置 WS_EX_LAYERED 扩展风格（启用分层窗口）
            GWL_EXSTYLE = -20
            WS_EX_LAYERED = 0x80000
            old = ctypes.windll.user32.GetWindowLongW(hwnd, GWL_EXSTYLE)
            ctypes.windll.user32.SetWindowLongW(
                hwnd, GWL_EXSTYLE, old | WS_EX_LAYERED)

            # 2. 颜色键抠图 + alpha 通道同时生效
            # COLORREF = 0x00BBGGRR, #010101 → 0x00010101
            # LWA_COLORKEY: TRANS_COLOR 像素 → 完全透明（alpha=0）
            # LWA_ALPHA:   其余像素 → alpha=254/255 ≈ opaque
            LWA_COLORKEY = 1
            LWA_ALPHA = 2
            ctypes.windll.user32.SetLayeredWindowAttributes(
                hwnd, 0x00010101, 254, LWA_COLORKEY | LWA_ALPHA)

            # 3. 消除 DWM 阴影 + 圆角（Win10/11）
        ...
        self._style.configure('TLabel', background=TRANS_COLOR, foreground='white')
        self._style.configure('TFrame', background=TRANS_COLOR)
        self._build_input()
        self._build_display()

        # 右键菜单（需在 UI 构建完毕后再建，因为依赖 _apply_theme）
        self._menu = tk.Menu(root, tearoff=0, font=('Microsoft YaHei', 10))
    ...
    def _apply_theme(self, name):
        ...
        ago = self.root.winfo_pointerx()

        # ---- 页面背景（始终透明） ----
        self.root.configure(bg=TRANS_COLOR)
        for w in self._tw.get('page_bg', []):
            if w.winfo_exists():
                w.configure(bg=TRANS_COLOR)

        # ---- 标题 / 副标题（浮于透明背景） ----
        for w in self._tw.get('title', []):
            if w.winfo_exists():
                w.configure(bg=TRANS_COLOR, fg=T['age'])
        for w in self._tw.get('subtitle', []):
            if w.winfo_exists():
                w.configure(bg=TRANS_COLOR, fg=T['text_dim'])
    ...
    def _card(self, parent, title, icon=''):
        """带边框的卡片，返回 grid 子容器"""
        outer = tk.Frame(parent, bg=TRANS_COLOR)
        outer.pack(fill=tk.X, pady=(0, 8))
        self._tag('page_bg', outer)
        body = tk.Frame(outer, bg='#141A3A',
                        highlightbackground='#1E2A5A',
    ...
    def _build_input(self):
        f = self.input_frame = tk.Frame(self.root, bg=TRANS_COLOR,
                                        padx=10, pady=10)
        self._tag('page_bg', f)
        ...
        lbl1 = tk.Label(f, text="🧬 生命记录仪",
                        ...
                        bg=TRANS_COLOR, fg='#00E5FF')
        lbl1.pack(pady=(0, 2))
        self._tag('page_bg', lbl1)
        self._tag('title', lbl1)

        lbl2 = tk.Label(f, text="桌面挂件 · 多主题炫彩",
                        font=('Microsoft YaHei', 9),
                        bg=TRANS_COLOR, fg='#5A6090')
        lbl2.pack(pady=(0, 10))
    ...
    def _build_display(self):
        f = self.display_frame = tk.Frame(self.root, bg=TRANS_COLOR,
                                          padx=10, pady=10)
        self._tag('page_bg', f)

        # ---- 年龄卡片 ----
        ac = tk.Frame(f, bg='#141A3A',
                      highlightbackground='#1E2A5A',
                      highlightthickness=1, padx=10, pady=8)
        ...
        for idx, (key, emoji, name, unit, color) in enumerate(stat_info):
            row = tk.Frame(sc, bg='#141A3A')
            row.pack(fill=tk.X, pady=1)
            self._tag('card_bg', row)
            el = tk.Label(row, text=emoji, font=('Microsoft YaHei', 11),
                          bg='#141A3A')
